...
export function formatText(pElement) {
    ...
    paragraphs.forEach((p, i) => {
        if (i == 0) lines.push(...paragraphToLines(pElement, p, { firstWord: 0, lastWord: 8 }))
        else lines.push(...paragraphToLines(pElement, p))
    })
...
export function paragraphToLines(pElement, text, sliceTitle) {
    ...
    if (sliceTitle) {
        let title = words.splice(sliceTitle.firstWord, sliceTitle.lastWord).filter(w => w != '*nl').join(' ')
        lines.push(title, '<br>')
    }
